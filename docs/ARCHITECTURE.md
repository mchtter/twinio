...
- **Araçlar**: tek `InstancedMesh` (kapasite 140). Kenar üzerinde ilerler, düğümde rastgele dönüş (oneway'e saygılı), yol sınıfına göre hız. Kamera yakınında doğar, uzaklaşınca geri dönüştürülür. **Boylamsal dinamik tam IDM'dir** (a=2.5, b=3, T=1.4 s, s₀=2 m; acil fren -9): önder araç, kırmızı/sarı sinyal (24 m grid sorgusu) ve üzerinde yaya olan zebra aynı yasadan geçer — en kısıtlayıcı ivme kazanır. Sinyal/zebra duran sanal önder olarak modellenir; kuyruklar s₀ aralıklı ve çarpışmasız istiflenir. Zebra yoklaması taşıt yolu yarı genişliği + 0.6 m yarıçapladır — kaldırımdaki yaya trafiği durdurmaz; geçidin üstündeki araç sorgudan muaftır (kilitlenme olmaz). **Şeritler**: yön-başı şerit sayısı OSM `lanes` etiketinden (yoksa genişlik/3.4'ten, 1-3 kelepçeli); şerit i merkezi `halfW − laneW·(i+0.5)`. **Şerit değiştirme** MOBIL-esintili: hedef şeritteki önder/takipçi boşlukları güvenliyse (takipçiye dayatılan fren > -b) ve kazanç eşiği aşılıyorsa sola sollama, maliyetsizse sağa dönüş; geçiş 1.4 sn'de smoothstep ile yanal süzülür, süresince araç iki şeridi birden işgal eder; düğümde değişim tamamlanmış sayılır ve şerit yeni kenarın sayısına kelepçelenir.
...
- **Trafik yoğunluğu** (`scenario/trafficFlow.ts`, HUD "Senaryo" paneli): şehir tek tıkla holograma döner ve sürüş grafı ışık fiberi olarak yeniden çizilir.
  - **Holo görünüm**: paylaşılan malzemeler yerinde mutasyona uğrar (`setHoloLook` — snapshot + tam geri yükleme). Binalar %22 opak hayalet cam (pencere emissive haritası camgöbeği yanar), bitki örtüsü silüet, zemin/yollar lacivert. Senaryo sırasında akan yeni karolar paylaşılan malzemeleri kullandığından otomatik temalı gelir. Ortam: gökyüzü gizlenir, arka plan/sis #040914, soğuk mavi ışık, gölgeler kapalı (`Environment.setHolo`); saat kaydırıcısı kaydedilir ama çıkışa dek uygulanmaz.
  - **Akış fiberleri**: kenar+yön başına bir şerit (çift yönlü yollarda araçlar gibi sağa ofsetli iki şerit). Custom shader: 16 m dalga boylu kuyruklu ışık darbeleri seyir yönünde akar — **darbe hızı o kenardaki canlı ortalama araç hızı, renk akıcılık oranı** (avgV/serbest hız: camgöbeği→kehribar→kırmızı), **parlaklık araç yoğunluğu**. Veri ajan simülasyonunun kendisidir (`VehicleSystem.edgeFlows`, 0.4 sn'de bir yenilenir): kırmızıda biriken kuyruk gerçek zamanlı kırmızı-sürünen fiber olarak görünür. Boş yollar soluk serbest-akış darbesi taşır. Geometri graf sürümü değişince yeniden kurulur; additive blend + `depthTest:false` (log-depth tamponuyla uyumsuzluk + hologram önünde/arkasında parlasın diye) + `renderOrder` ile en üstte çizilir.

- **Altyapı (yeraltı)** (`scenario/infrastructure.ts`): aynı holo görünümde şehrin altı. OSM'de gerçek dağıtım şebekesi yok denecek kadar azdır (belediye GIS'inde yaşar — Faz 5 içe aktarımı aynı render'a gerçek geometri besleyecek); şebeke cadde ağından SENTEZLENİR — gerçek şebekeler de caddeyi izler: her yolun altında kanalizasyon ana hattı (~2.6 m, toksik yeşil, darbeler YOKUŞ AŞAĞI — yerçekimi) + sağa ofsetli basınçlı su hattı (~1.4 m, elektrik mavisi, hızlı darbe). Rögarlar kavşaklarda + kanal boyunca ~45 m arayla (6 m grid dedupe) camgöbeği halkalar. Gerçek OSM verisi bindirilir: `man_made=pipeline` magenta hat (substance/location etiketli), `manhole` rögarlar, `emergency=fire_hydrant` turuncu direkler. Fiberler aynı shader'ı paylaşır (statik renk/hız, `depthTest:false` → yeraltı yerin üstünden okunur). Senaryolar HUD'da tek seçimlidir; Overpass sorgusuna eklenen altyapı öğeleri `osmCacheVersion` 3'e taşıdı.
